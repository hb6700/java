...
public class Ex26_DateTime {
	...
	private static void m7() {
		...
		//2시간 40분 + 30분 = 2시간 70분, 3시간 10분(X)
		//hour = 2; min = 40;
		min += 30;
		System.out.printf("%d시간 %d분\n", hour, min); //2시간 70분
		
		//2시간 70분 -> 3시간 10분 (자리올림을 잘하라!)
	...
	private static void m6() {
		...
		//1년은 365일이 아님!! 365일로 하면 근사치는 만들어낼 수 있으나, 정확하진 않음. 여기서는 일단 365로 계산! 월도 일단 30일로 근사치로 계산!
	...
	private static void m2() {
		...
		System.out.println(c1.get(Calendar.DAY_OF_YEAR)); 		//올해 몇 일? : 201(일) > 올해 들어 201일 째
		System.out.println(c1.get(Calendar.DAY_OF_MONTH)); 		//이번달 몇 일? : 20(일) > 이번달 들어 20일 째
		System.out.println(c1.get(Calendar.DAY_OF_WEEK)); 		//5 > 요일 > 1(일) ~ 7(토) > 이번주 들어 5일째 //반환값이 int라 숫자로 돌려줌.
		System.out.println(c1.get(Calendar.WEEK_OF_YEAR)); 		//29(주차) > 올해들어 29주차
